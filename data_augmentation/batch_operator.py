...
from data_augmentation.batch_retriever import retrieve_batch


NUM_PARALLEL_BATCHES = 0
POLL_INTERVAL_SECONDS = 10
MAX_CYCLES = 10000
ACTIVE_LOG_PATH = Path("data_augmentation/batch_id_logs.jsonl")
DONE_LOG_PATH = Path("data_augmentation/batch_id_logs_done.jsonl")
ACTIVE_STATUSES = {
    "validating",
    "in_progress",
    "finalizing",
...
def run_batch_generator(count: int) -> None:
    for idx in range(count):
        print(f"[batch_operator] generating batch {idx + 1}/{count}")
        subprocess.run(
            [sys.executable, "-m", "data_augmentation.batch_generator"],
            check=True,
        )
...
def process_active_batches(client: OpenAI) -> tuple[list[dict], list[dict]]:
    active_records = read_jsonl(ACTIVE_LOG_PATH)
    ...
    for record in active_records:
        ...
        status = batch.status
        print(f"[batch_operator] batch_id={batch_id} status={status}")
        ...
        if status in TERMINAL_STATUSES:
            retrieved = retrieve_batch(batch_id, client=client)
            archived_record = dict(record)
            archived_record["final_status"] = status
            archived_record["output_file_id"] = batch.output_file_id
            archived_record["error_file_id"] = batch.error_file_id
            ...
            print(
                f"[batch_operator] archived batch_id={batch_id} "
...
def main() -> None:
    ...
    client = OpenAI()
    print(f"[batch_operator] target_parallel_batches={NUM_PARALLEL_BATCHES}")
    if NUM_PARALLEL_BATCHES > 0:
        top_off_batches()
    else:
        active_records = read_jsonl(ACTIVE_LOG_PATH)
        print(
            "[batch_operator] top-off disabled; "
    ...
    while True:
        cycles += 1
        print(f"[batch_operator] cycle={cycles}")
        ...
        if NUM_PARALLEL_BATCHES > 0:
            ...
            print("[batch_operator] no active batches remain; exiting")
            break
        if MAX_CYCLES is not None and cycles >= MAX_CYCLES:
            print(f"[batch_operator] reached MAX_CYCLES={MAX_CYCLES}, exiting")
            break
